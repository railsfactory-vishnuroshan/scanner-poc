...
const BarcodeScanner: React.FC<BarcodeScannerProps> = ({ onScan, isOpen }) => {
      ...
      if (videoInputDevices.length > 0) {
        if (videoInputDevices[currentCameraIndex]) {
          ...
          console.log(
            ...
            videoInputDevices[currentCameraIndex].label
          );
        } else {
  ...
  const switchCamera = useCallback(async () => {
    ...
    console.log(
      `🔄 Switching from camera ${currentCameraIndex} to ${nextIndex}`
    );
          ...
          <div className="relative">
            ...
            {availableCameras.length > 1 && isScanning && (
              <button
                ...
                className="absolute right-3 bottom-3 rounded-full bg-black/50 p-2 text-white hover:bg-black/70 disabled:opacity-50 transition-colors"
                title={`Switch Camera (${currentCameraIndex + 1}/${
